...
reviews = pd.read_csv("./input/reviews_l12m.csv", header = 0, encoding = 'latin-1')
...
reviews["comments_cleaned"] = reviews["comments_cleaned"].str.split().apply(lambda x: [stemmer.stem(y) for y in x])
...
def draw_wordcloud(dict, topic_number):
    ...
    wordcloud = WordCloud().generate_from_frequencies(dict).to_file("output/stemmer/topic_" + str(topic_number) + ".png")
